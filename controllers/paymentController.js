...
exports.verifyPayment = async (req, res) => {
  try {
    ...
    if (!user) {
      user = new User({ email, unlockedPdfs: [], payments: [] });
    }

    // unlock pdf
    const alreadyUnlocked = user.unlockedPdfs.find(p => p.pdfId === pdfId);
...
exports.getPdfLink = async (req, res) => {
  try {
    ...
    const isUnlocked = user.unlockedPdfs.find(p => p.pdfId === pdfId);

    if (!isUnlocked) {
      return res.status(403).json({ message: "Not unlocked" });
    }

    // file mapping (simple version)
    const fileMap = {
